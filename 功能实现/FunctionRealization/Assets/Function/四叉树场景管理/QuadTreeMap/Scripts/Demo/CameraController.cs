...
namespace WCC.QuadTree
{
    public class CameraController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            cameraPositionTemp = new Vector3(Mathf.Clamp(cameraPositionTemp.x, moveLimitX[0], moveLimitX[1]), Mathf.Clamp(cameraPositionTemp.y, zoomMinPositionY, zoomMaxPositionY),Mathf.Clamp(cameraPositionTemp.z, moveLimitZ[0], moveLimitZ[1]));
            float rotateX = (cameraPositionTemp.y - zoomMinPositionY) / (zoomMaxPositionY - zoomMinPositionY) * (zoomMaxRotateX - zoomMinRotateX) + zoomMinRotateX;
        ...
        [SerializeField] private float zoomScaleCo = 0.1f; //缩放系数
        private void Zoom()
        {
